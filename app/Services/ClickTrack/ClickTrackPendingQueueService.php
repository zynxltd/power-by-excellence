...
class ClickTrackPendingQueueService
{
    ...
    {
        $query = TrackingConversion::query()
        ...
        $items = $query->get()->map(fn (TrackingConversion $c) => [
            'id' => $c->id,
    ...
    /**
     * Links at or over click/conversion caps.
     *
    ...
    public function capAlerts(?int $accountId, int $limit = 10): array
    {
        ...
        return $links
            ->map(fn (TrackingLink $link) => $this->caps->usageForLink($link))
            ->filter(fn (array $usage) => $usage['click_cap_reached'] || $usage['conversion_cap_reached'])
            ->take($limit)
            ->values()
            ->all();
    }
