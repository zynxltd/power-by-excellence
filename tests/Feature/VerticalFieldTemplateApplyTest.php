...
class VerticalFieldTemplateApplyTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        \Illuminate\Support\Facades\Route::get(
            '/vertical-field-templates/apply-wizard',
            [\App\Http\Controllers\Admin\VerticalFieldTemplateController::class, 'applyWizard'],
        )->middleware(['web', 'auth'])->name('vertical-field-templates.apply-wizard');

        \Illuminate\Support\Facades\Route::post(
            '/vertical-field-templates/{verticalFieldTemplate}/preview',
            [\App\Http\Controllers\Admin\VerticalFieldTemplateController::class, 'preview'],
        )->middleware(['web', 'auth'])->name('vertical-field-templates.preview');
    ...
    public function test_preview_does_not_persist_campaign_fields(): void
    {
        ...
        $this->ukHost()
            ->actingAs($this->admin)
            ->postJson('/vertical-field-templates/'.$template->id.'/preview', [
                'campaign_id' => $this->campaign->id,
                'strategy' => VerticalFieldTemplateApplyService::STRATEGY_REPLACE_ALL,
            ])
            ->assertOk()
    ...
    public function test_wrong_vertical_is_rejected_for_preview_and_apply(): void
    {
        ...
        $this->ukHost()
            ->actingAs($this->admin)
            ->from('/vertical-field-templates/apply-wizard?campaign_id='.$this->campaign->id)
            ->post(route('vertical-field-templates.apply', $template), [
                'campaign_id' => $this->campaign->id,
            ])
            ->assertRedirect()
    ...
    public function test_apply_wizard_page_filters_templates_by_campaign_vertical(): void
    {
        ...
        $this->ukHost()
            ->actingAs($this->admin)
            ->get('/vertical-field-templates/apply-wizard?campaign_id='.$this->campaign->id)
            ->assertOk()
