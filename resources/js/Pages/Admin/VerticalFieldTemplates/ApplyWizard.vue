...
const { currentStep, goStep, stepStatus, prevStep } = useFormSteps(steps);
...
const loadPreview = async () => {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            previewError.value = Object.values(errors).flat().join(' ') || payload?.message || 'Unable to load preview.';
            return;
        }
...
<template>
    ...
    <AuthenticatedLayout>
        ...
        >
            <template #actions>
                <AppButton v-if="campaign" variant="secondary" :href="route('campaigns.show', campaign.id)">Back to campaign</AppButton>
        ...
        <Panel v-show="currentStep === 'template'" title="1. Choose template and campaign">
            <div class="grid gap-4 md:grid-cols-2">
                <div>
                    ...
                    <input v-model="selectedCampaignId" type="number" class="form-input mt-1 w-full" :readonly="Boolean(campaign)" required />
        ...
        <Panel v-show="currentStep === 'confirm'" title="3. Confirm apply">
            ...
            <div class="mt-6 flex justify-between gap-3">
                ...
                <PrimaryButton :disabled="applyForm.processing" @click="submitApply">Apply template</PrimaryButton>
            </div>
        </Panel>
    </AuthenticatedLayout>
</template>
