...
const props = defineProps({
    billing: Object,
    account: Object,
    portalType: { type: String, default: 'buyer' },
});

const isBuyer = computed(() => props.portalType === 'buyer');
const billingRoute = computed(() => (
    isBuyer.value ? route('portal.buyer.billing') : route('portal.supplier.billing')
));
</script>
    ...
    <AuthenticatedLayout>
        <div class="mx-auto max-w-2xl py-12 text-center">
            ...
            <p class="mt-2 text-slate-600 dark:text-slate-400">
                <strong>{{ account?.display_name ?? account?.name }}</strong> is suspended until billing is resolved.
                Lead delivery and most portal features are unavailable.
            </p>
            ...
            <div class="mt-8 flex flex-wrap justify-center gap-3">
                <AppButton :href="billingRoute">View billing</AppButton>
                <AppButton variant="secondary" :href="route('support.create')">Contact support</AppButton>
                <Link :href="route('support.index')" class="text-sm font-medium text-indigo-600 hover:text-indigo-500 dark:text-indigo-400">
                    Support tickets
                </Link>
            </div>
        </div>
